...
CREATE TABLE IF NOT EXISTS songplays (songplay_id serial PRIMARY KEY, start_time bigint NOT NULL, user_id varchar NOT NULL,
level varchar, song_id varchar, artist_id varchar, session_id int, location varchar, user_agent varchar
)
"""

user_table_create = """
CREATE TABLE IF NOT EXISTS users (user_id serial PRIMARY KEY , first_name varchar, last_name varchar, gender varchar, level varchar)
"""
...
song_select = """
SELECT s_songs.song_id, a_artists.artist_id FROM songs as s_songs 
JOIN artists as a_artists ON s_songs.artist_id = a_artists.artist_id 
WHERE s_songs.title = %s AND a_artists.name = %s AND s_songs.duration = %s 
"""
